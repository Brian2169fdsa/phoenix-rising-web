...
const STEP_DESCS = ['Pick a service', 'Tell us the details', 'Where in the valley', 'Day & time', 'Your info', 'Confirm & price'];
...
const SIZE_BASE: Record<string, number> = { small: 189, medium: 249, large: 329 };
const SOLAR_BASE = 149;
const SCREENS_BASE = 59;
...
function estimate(d: BookingData): number | null {
...
function MonthCal({ value, onPick }: { value: number | null; onPick: (d: number) => void }) {
  const days = 30;
  const firstDow = 1; // June 2026 starts on Monday
  const today = 6;
    ...
    <div>
      ...
      <div className="cal">
        ...
        {cells.map((d, i) => {
          ...
          const disabled = d < today || dowIdx === 0;
          return (
            <button
              key={i}
              className={`cal__day${value === d ? ' sel' : ''}`}
              disabled={disabled}
              onClick={() => onPick(d)}
            >
              {d}
...
export default function BookingPage() {
  ...
  const [d, setD] = useState<BookingData>({
    service: '', size: '', stories: '', windows: '', sqft: '', floors: '',
    frequency: 'one-time', hardwater: false, panels: '', street: '', city: '',
    zip: '', date: null, slot: '', name: '', email: '', phone: '', notes: '',
  });

  const set = (k: keyof BookingData, v: string | boolean | number | null) =>
    setD((p) => ({ ...p, [k]: v }));
  ...
  function review() {
    if (instant) {
      ...
      router.push('/checkout/success?type=quote&service=' + d.service);
    }
  }
    ...
    <div className="site">
      ...
      <section className="pagehead">
        <div className="container pagehead__inner" style={{ padding: '56px 0' }}>
          <div className="crumbs"><a href="/">Home</a> / <span>Book</span></div>
          <Eyebrow>Book a Cleaning</Eyebrow>
          <h1 className="pagehead__title" style={{ fontSize: 'clamp(34px,4.5vw,48px)' }}>
            Crystal-clear in <span className="accent">six steps</span>
          </h1>
        </div>
      </section>
              ...
              <ol className="bk__steps">
                {STEPS.map((s, i) => (
                  <li key={s} className={`bk__step${i === step ? ' on' : i < step ? ' done' : ''}`}>
                    <span className="bk__num">
                      {i < step ? <Icon name="check" size={15} /> : i + 1}
                    </span>
                    <span>
                      <span className="bk__step-t">{s}</span>
                      <span className="bk__step-d">{STEP_DESCS[i]}</span>
                    </span>
                  </li>
                ))}
              </ol>
            ...
            <div>
              ...
              <div className="panel">
                  ...
                  <div>
                    ...
                    <div className="optgrid optgrid--2" style={{ marginTop: 22 }}>
                      {SERVICES.map((s) => (
                        <button
                          key={s.id}
                          className={`opt${d.service === s.id ? ' sel' : ''}`}
                          onClick={() => set('service', s.id)}
                        >
                          <span className="opt__ico"><Icon name={s.icon} size={20} /></span>
                          <span>
                            <span className="opt__t">{s.t}</span>
                            <span className="opt__d">{s.d}</span>
                            {s.instant && (
                              <span style={{ display: 'inline-block', marginTop: 6 }}>
                                <Badge tone="success">Instant price</Badge>
                              </span>
                            )}
                          </span>
                        </button>
                      ...
                      <div className="field-stack" style={{ marginTop: 22 }}>
                        <div>
                          ...
                          <div className="optgrid optgrid--3">
                            {[['small', 'Small', '≤ 20 panes'], ['medium', 'Medium', '20–35 panes'], ['large', 'Large', '35+ / estate']].map(([v, t, sub]) => (
                              <button
                                key={v}
                                className={`opt${d.size === v ? ' sel' : ''}`}
                                style={{ display: 'block' }}
                                onClick={() => set('size', v)}
                              >
                                <span className="opt__t">{t}</span>
                                <span className="opt__d">{sub}</span>
                              </button>
                            ))}
                          </div>
                        </div>
                        ...
                        <label style={{ display: 'flex', gap: 10, alignItems: 'center', fontSize: 14.5, cursor: 'pointer' }}>
                          <input
                            type="checkbox"
                            checked={d.hardwater}
                            onChange={(e) => set('hardwater', e.target.checked)}
                            style={{ width: 18, height: 18, accentColor: 'var(--phoenix-blue)' }}
                          />
                    ...
                    <div className="field-stack" style={{ marginTop: 22 }}>
                      ...
                      {d.city && /^85\d{3}$/.test(d.zip) && (
                        <div className="securenote">
                          <Icon name="check-circle" size={16} /> Great news, {d.city} is right in our service area.
                        </div>
                      )}
                    </div>
                  </div>
                ...
                {step === 3 && (
                  <div>
                    ...
                    <div className="split" style={{ marginTop: 22, gap: 32, alignItems: 'start' }}>
                      <MonthCal value={d.date} onPick={(v) => set('date', v)} />
                      <div>
                ...
                {step === 5 && (
                  <div>
                    ...
                    <p className="muted" style={{ marginTop: 8 }}>
                      {instant ? "Here's your instant price. Next stop: secure checkout." : "We'll send this to our estimators for a fast, free quote."}
                    </p>
                    <div style={{ marginTop: 20 }}>
                      {([
                        ['Service', svc ? svc.t : '-'],
                        ['Property', d.service === 'residential' ? `${d.size || '-'} home · ${d.stories || '1'} story` : d.sqft ? `${d.sqft} sq ft` : d.panels ? `${d.panels} panels` : '-'],
                        ['Frequency', d.frequency],
                        ['Address', `${d.street}, ${d.city} ${d.zip}`],
                        ['Date', d.date ? `June ${d.date}, 2026` : '-'],
                        ['Window', d.slot || '-'],
                      ...
                      {!instant && (
                        <p className="muted" style={{ fontSize: 13, marginTop: 10 }}>
                          Commercial, high-rise, and post-construction jobs are quoted on-site so you only pay for what you need.
                        </p>
                      )}
                    </div>
                  </div>
                ...
                <div className="bk__actions">
                  {step > 0
                    ? <Button variant="tertiary" onClick={() => go(step - 1)} leftIcon="arrow-left">Back</Button>
                    : <span className="muted" style={{ fontSize: 13 }}>Takes about a minute</span>}
                  {step < STEPS.length - 1
                    ? <Button variant="primary" disabled={!canNext[step]()} onClick={() => go(step + 1)} rightIcon="arrow-right">Continue</Button>
                    : <Button variant="primary" onClick={review} rightIcon={instant ? 'credit-card' : 'send'}>{instant ? 'Continue to checkout' : 'Request my quote'}</Button>}
                </div>
              </div>

              <div className="securenote" style={{ justifyContent: 'center', marginTop: 18 }}>
                <Icon name="lock" size={15} /> Secure &amp; private · No charge until you confirm
              </div>
            </div>
          </div>
        </div>
