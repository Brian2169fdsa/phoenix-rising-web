...
(() => {
  ...
  class ImageSlot extends HTMLElement {
    ...
    connectedCallback() {
      ...
      // Re-frame on viewport resize so a responsive layout change re-seeds the
      // cover baseline. Uses a debounced window 'resize' listener instead of a
      // per-element ResizeObserver: an RO that re-renders can make the browser
      // emit the (benign but noisy) "ResizeObserver loop completed" warning when
      // many slots wake at once during a heavy reflow. A window listener avoids
      // that class of warning entirely while preserving the responsive behavior.
      this._onResize = () => {
        if (this._rzT) clearTimeout(this._rzT);
        this._rzT = setTimeout(() => { this._rzT = 0; this._render(); }, 150);
      };
    ...
    disconnectedCallback() {
      ...
      if (this._rzT) { clearTimeout(this._rzT); this._rzT = 0; }
      this._exitReframe(false);
    }
