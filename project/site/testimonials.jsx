const { Icon, SiteHeader, SiteFooter, Eyebrow, SectionHead, Stars, mount } = window;
...
const ini = (n) => n.split(/[^A-Za-z]+/).filter(Boolean).map((w) => w[0]).join("").slice(0, 2).toUpperCase();
...
function TestimonialsPage() {
  ...
  return (
    <div className="site">
      <SiteHeader active="testimonials.html" />
      <section className="pagehead">
        <div className="container pagehead__inner">
          <div className="crumbs"><a href="index.html">Home</a> / <span>Reviews</span></div>
      ...
      <section className="section--alt section--tight">
        <div className="container">
          <div className="panel" data-fade style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 28, flexWrap: "wrap" }}>
            <div className="ratingbig">
              <span className="ratingbig__num">4.9</span>
              <div><Stars value={5} size={20} /><p className="muted" style={{ fontSize: 14, marginTop: 4 }}>Average across 600+ completed jobs</p></div>
            </div>
            <div className="statrow" style={{ flex: 1, minWidth: 320 }}>
              {[["600+", "Jobs completed"], ["98%", "Would refer us"], ["100%", "Insured & bonded"], ["<24h", "Avg. quote time"]].map(([n, l]) => (
                <div className="stat" key={l}><div className="stat__num" style={{ fontSize: 34 }}>{n}</div><div className="stat__lbl">{l}</div></div>
              ))}
            </div>
          </div>
        </div>
      ...
      <section className="section">
        <div className="container">
          <div className="chips" style={{ marginBottom: 34, justifyContent: "center" }}>
            {FILTERS.map((c) => <button key={c} className={"chip" + (f === c ? " on" : "")} onClick={() => setF(c)}>{c}</button>)}
          </div>
          <div className="grid grid-3">
            {list.map((rv, i) => (
              <Card key={i} className="review" data-fade>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  <Stars value={rv.r} /><Badge tone="primary">{rv.cat}</Badge>
                </div>
                ...
                <div className="review__by">
                  ...
                  <span><span className="review__name">{rv.n}</span><br /><span className="review__meta">{rv.m} · Verified job</span></span>
                </div>
              </Card>
            ))}
          </div>
          <div className="note-row" style={{ marginTop: 36 }}><Icon name="star" size={16} /><span>Reviews mirrored from Google &amp; Nextdoor. We never edit or cherry-pick, the occasional 4-star keeps us honest.</span></div>
        </div>
      </section>
      ...
      <section className="section section--tight">
        <div className="container">
          <div className="ctaband glint glint--animate" data-fade>
            <div><h2>Join 600+ happy valley neighbors</h2><p>See the difference for yourself, book a clean or grab a free quote today.</p></div>
