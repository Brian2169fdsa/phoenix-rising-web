...
const PINS = [["Phoenix", 42, 50], ["Scottsdale", 60, 36], ["Paradise Valley", 54, 42], ["Tempe", 52, 60], ["Mesa", 66, 58], ["Chandler", 60, 70], ["Gilbert", 70, 68], ["Glendale", 30, 42], ["Peoria", 24, 32], ["Surprise", 14, 28]];
  ...
  return (
    <svg viewBox="0 0 100 78" style={{ width: "100%", height: "100%", display: "block" }} aria-label="Phoenix metro coverage map">
      ...
      <g stroke="#9db9d8" strokeWidth="0.35" opacity="0.6">
        <line x1="0" y1="40" x2="100" y2="40" /><line x1="0" y1="58" x2="100" y2="58" />
        <line x1="38" y1="0" x2="38" y2="78" /><line x1="64" y1="0" x2="64" y2="78" />
      </g>
      {/* coverage halo */}
...
function AddressCheck() {
  ...
  function check(e) { e.preventDefault(); setRes(/^85\d{3}$/.test(zip.trim())); }
  return (
    <form className="panel" onSubmit={check} data-fade>
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 18 }}>
        <div style={{ flex: 1 }}><Input placeholder="85016" value={zip} onChange={(e) => setZip(e.target.value)} /></div>
        <Button variant="primary" type="submit" rightIcon={chevron}>Check</Button>
      </div>
      {res === true && <div className="securenote" style={{ marginTop: 16 }}><Icon name="check-circle" size={16} /> You're in our service area, let's get you booked!</div>}
      {res === false && <div className="securenote" style={{ marginTop: 16, color: "var(--ember)" }}><Icon name="map-pin" size={16} /> That's outside our usual radius, call us, we may still help.</div>}
      {res === true && <div style={{ marginTop: 16 }}><Button variant="secondary" fullWidth href="booking.html">Book a cleaning</Button></div>}
    </form>
  );
}
...
function ServiceAreaPage() {
  return (
    <div className="site">
      <SiteHeader active="service-area.html" />
      <section className="pagehead">
        <div className="container pagehead__inner">
          <div className="crumbs"><a href="index.html">Home</a> / <span>Service Area</span></div>
      ...
      <section className="section">
        <div className="container split" style={{ gap: 44, alignItems: "start" }}>
          <div data-fade><div className="map"><MapArt /></div></div>
          <AddressCheck />
        </div>
      </section>
      ...
      <section className="section section--alt">
        <div className="container">
          ...
          <div className="grid grid-2 grid--mt">
            {ZONES.map((z) => (
              <Card key={z.city} data-fade style={{ display: "flex", gap: 16, alignItems: "flex-start" }}>
                ...
                <div>
                  ...
                  <div className="tag-list" style={{ marginTop: 10 }}>
                    {z.hoods.map((h) => <span key={h} className="kbd-pill" style={{ fontSize: 12.5, padding: "5px 11px" }}>{h}</span>)}
                  </div>
                </div>
              </Card>
            ))}
      ...
      <section className="section section--tight">
        <div className="container">
          <div className="ctaband glint glint--animate" data-fade>
            <div><h2>In the valley? You're covered.</h2><p>Book online in minutes or grab a free same-day quote.</p></div>
